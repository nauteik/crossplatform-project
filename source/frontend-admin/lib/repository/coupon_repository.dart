  ...
  Future<List<Coupon>> fetchCoupons() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Không thể tải danh sách mã giảm giá. Lỗi: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<Coupon> addCoupon(String code, int value, int maxUses) async {
    ...
    try {
      final response = await http.post(
        uri,
        headers: _headers,
        body: jsonEncode(tempCoupon.toJsonForCreation()), // <-- Sử dụng toJsonForCreation
      );
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        throw Exception('Không thể thêm mã giảm giá. Lỗi: ${response.statusCode} - ${utf8.decode(response.bodyBytes)}');
      }
    } catch (e) {
  ...
  // Thêm phương thức xóa coupon DELETE /api/coupon/{id}
   Future<void> deleteCoupon(String couponId) async {
      ...
      try {
         ...
         if (response.statusCode == 200 || response.statusCode == 204) { // Assuming 200 OK or 204 No Content on success
            print('Coupon $couponId deleted successfully');
         } else {
            print('Failed to delete coupon $couponId: ${response.statusCode}');
            print('Response body: ${utf8.decode(response.bodyBytes)}');
            throw Exception('Không thể xóa mã giảm giá $couponId. Lỗi: ${response.statusCode}');
         }
      } catch (e) {
